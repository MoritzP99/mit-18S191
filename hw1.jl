...
student = (name = "Moritz Pail", kerberos_id = "moritz.pail")
...
round(mean(demean(copy_of_random_vect)))
...
function vecvec_to_matrix(vecvec)
	n_rows, n_columns = length(vecvec), length(vecvec[1])
	M = zeros(n_rows, n_columns)
	for row in 1:n_rows 
		for column in 1:n_columns
			M[row, column] = vecvec[row][column]
		end
	end
	return M
end
...
function mean_colors(image)
	mean_values = reduce(+, image) / length(image)
	return (mean_values.r, mean_values.g, mean_values.b)
end
...
# ╔═╡ e31ac238-3e54-11eb-13db-b74469c82a67
...
# ╔═╡ bcaf0820-3ef2-11eb-16d1-3b7624e7a2c3
function myclamp(x, start_idx, end_idx)
	return max(start_idx, min(end_idx, x))
end

# ╔═╡ f6e2cb2a-ee07-11ea-06ee-1b77e34c1e91
begin
	function noisify(x::Number, s)
		return myclamp(x + rand(-s:s), 0, 1)
	end
...
The image is unrecognisable around intensity 2.1
"""
...
@bind l_box Slider(0:0.1:1, show_value=true)

# ╔═╡ 519874d0-3ef8-11eb-1f8b-3755019346c8
colored_line(v)

# ╔═╡ 64f77fd2-3ef8-11eb-383a-13217a48c275
blurred_v = blur_1D(v, l_box)

# ╔═╡ f000848c-3efa-11eb-25e6-c96885560ce0
...
function convolve_vector(v, k)
	...
	mid = (length(k) + 1) ÷ 2
	new_v = zeros(size(v))
	for i = 1:length(v), n = -l:l
		new_v[i] += extend(v, i-n) * k[mid+n]
	end
	return new_v
end
...
function extend_mat(M::AbstractMatrix, row, column)
	num_rows, num_columns = size(M)
	return M[myclamp(row, 1, num_rows), myclamp(column, 1, num_columns)]
end
...
# ╔═╡ 4acea742-3f94-11eb-1bf5-5d338e602920
md"""
...
# ╔═╡ 8a94d9a4-3f97-11eb-1ad7-3f190051e008


# ╔═╡ 8b96e0bc-ee15-11ea-11cd-cfecea7075a0
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
    num_rows, num_cols = size(M)
	copied_matrix = copy(M)
    kernel_row_length, kernel_col_length = (size(K) .- 1) .÷ 2
	
	# helper function to compute single convolution
    conv = (matrix, row, col) -> reduce(+, 
		[
            extend_mat(matrix, height + row, width + col) 
                for height in -kernel_row_length:kernel_row_length, 
                    width in -kernel_col_length:kernel_col_length
		] .* K)
    
	# apply convolution to the whole matrix
    return [conv(copied_matrix, mrow, mcol) 
			for mrow in 1:num_rows, mcol in 1:num_cols]
end
...
begin
    function gaussian_filter(n::Int, σ::Number)
        G(x,y,σ) = 1/2π*σ^2 *ℯ^(-(x^2 + y^2)/(2σ^2))
        ν = (n + 1) ÷ 2
        M = fill(0., n,n)
        [M[i,j] = G(i-ν, j-ν, σ) for i in 1:n for j in 1:n]
        return (M ./ sum(M))
    end

    function with_gaussian_blur(image, n::Int, σ::Number)
        filter = gaussian_filter(n, σ)
        return convolve_image(image, filter)
    end
end
...
with_gaussian_blur(philip, 11, 4)
...
# ╔═╡ 22d5e8da-4044-11eb-2f5f-cb85ed737c44
begin
...
function with_sobel_edge_detect(image)
	...
	comb(img1, img2) = square_root.(square.(Gx) + square.(Gy))
