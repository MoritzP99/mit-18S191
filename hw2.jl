...
student = (name = "Moritz", kerberos_id = "moritz.pail")
...
342. Everytime the for-loop runs with vcat() it makes a copy of the row, so the number of allocations that we reduced is #steps in the for-loop or length of the image.
"""
...
views_observation = md"""
	684. Every slice of an array corresponds to 1 allocation. The algorithm does 2 		slices per loop, and runs 342 times. Hence we get 2 * 342 + 684.
"""
...
# ╔═╡ ec7a9fae-41ff-11eb-0805-85efd253f121
potential_cols = [5] .+ [-1, 0, 1]

# ╔═╡ 91121fe8-4202-11eb-0868-f7dbfd232381
5 == findmax(clamp.(potential_cols, 0, 5))[1]

# ╔═╡ dd33c068-4205-11eb-226a-e922b6085231
potential_cols[2]

# ╔═╡ abf20aa0-f31b-11ea-2548-9bea4fab4c37
function greedy_seam(energies, starting_pixel::Int)
	seam = zeros(Int64, size(energies, 1)) # initialize seam-array with width of 													energies
	
	seam[1] = starting_pixel # store first pixel 
	
	for i=2:size(energies, 1)
		# find possible indices for neighbors, clamping them if they are out of 				range
		potential_cols = clamp.((seam[i-1] .+ [-1, 0, 1]), 1, size(energies, 2))
		
		# set seam[i] to the minimum of possible neighbors
		seam[i] = potential_cols[findmin(@view energies[i,potential_cols])[2]]
	end
	return seam
end
...
# ╔═╡ 03b32654-41ff-11eb-0206-d39405f874d1
y = findall(x -> x == 7, greedy_test)
